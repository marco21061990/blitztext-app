...
supported frontmost provider is preferred; for a menu start, the provider
selection context is captured before the Blitztext popover is activated.
Without a supported frontmost provider, the coordinator requires a complete
inspection and explicit source identity for every playing provider. Each
confirmation is independently owned and briefly polled within the same bounded
path. The setting **Wiedergabe automatisch pausieren** is
